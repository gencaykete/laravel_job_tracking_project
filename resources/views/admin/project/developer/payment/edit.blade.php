                ...
                <div class="card-body p-lg-17">
                    ...
                    <div class="d-flex flex-column flex-lg-row mb-17">
                        <div class="flex-lg-row-fluid me-0 me-lg-20">
                            <form action="{{route('admin.project-developer-payment.update',$projectDeveloperPayment->id)}}" class="form mb-15" method="post"
                                    ...
                                    <div class="col-md-6 col-12 fv-row">
                                        ...
                                        <select name="developer_id" class="form-select" data-placeholder="Yazılımcı Seçiniz" data-control="select2" data-hide-search="true">
                                            ...
                                            @foreach($developers as $row)
                                                <option value="{{$row->id}}" @selected($projectDeveloperPayment->project_developer_id == $row->id)>{{$row->name.' '.$row->surname}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-6 col-12 fv-row">
                                        ...
                                        <input type="text" class="form-control form-control-solid" placeholder=""
                                               required name="amount" value="{{old('amount') ?? $projectDeveloperPayment->amount}}"/>
                                    </div>
                                </div>
                                ...
                                <div class="row mb-5">
                                    <div class="col-md-6 col-12 fv-row">
                                        ...
                                        <select name="type" class="form-select" data-placeholder="Ödeme Tipi Seçiniz" data-control="select2" data-hide-search="true">
                                            <option value="0" @selected($projectDeveloperPayment->type == 0)>Yapılan Ödeme</option>
                                            <option value="1" @selected($projectDeveloperPayment->type == 1)>Ekleme</option>
                                            <option value="2" @selected($projectDeveloperPayment->type == 2)>Kesinti</option>
                                        </select>
                                    </div>

                                    <div class="col-md-6 col-12 fv-row">
                                        <label class="fs-5 fw-bold mb-2">Açıklama</label>
                                        <input type="text" class="form-control form-control-solid" name="description" value="{{old('description') ?? $projectDeveloperPayment->description}}"/>
                                    </div>
                                </div>

                                <div class="row mb-5">
                                    <div class="col-md-12 col-12 fv-row">
                                        <label class="fs-5 fw-bold mb-2">Dekont @if($projectDeveloperPayment->file) <a href="{{storage($projectDeveloperPayment->file)}}">(Görüntüle)</a> @endif</label>
